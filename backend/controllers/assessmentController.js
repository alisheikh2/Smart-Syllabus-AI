...
const createAssessment = async (req, res) => {
  try {
    ...
    if (error instanceof AIServiceUnavailableError) {
      console.warn("Gemini unavailable — saving fallback assessment sample");
      const totalMarks =
        (mcqCount || 0) * (mcqMarks || 0) +
        (shortCount || 0) * (shortMarks || 0) +
        (longCount || 0) * (longMarks || 0);
      ...
      const assessment = await Assessment.create({
        ...
        totalMarks,
        mcqs: fallbackSamples.assessment.mcqs,
        shortQuestions: fallbackSamples.assessment.shortQuestions,
        longQuestions: fallbackSamples.assessment.longQuestions,
        createdBy: email,
        coveredWeeks: weeks || [],
        difficultyDistribution: { easyPercent, mediumPercent, hardPercent },
        isFallback: true,
      ...
      return res
        .status(201)
        .json({ success: true, isFallback: true, assessment });
    }
...
const updateAssessment = async (req, res) => {
  try {
    ...
    if (shortQuestions !== undefined)
      assessment.shortQuestions = shortQuestions;
